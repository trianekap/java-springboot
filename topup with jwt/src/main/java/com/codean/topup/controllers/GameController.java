...
public class GameController {
    ...
    public ApiResponse<PageResult<GameDto>> getAllByPage(
            @RequestParam(defaultValue = "1") int page,
            @RequestParam(defaultValue = "10") int pageSize) {

        PageResult<GameDto> list;

        try {
            list = gameService.getAllByPage(page, pageSize);
    ...
    @PostMapping("/create")
    public ApiResponse<Object> insert(@RequestBody @Valid GameDto gameDto) {
    ...
    public ApiResponse<Object> update(@PathVariable Long id, @RequestBody @Valid GameDto gameDto){
